...
function editarUsuario(id) {
    fetch('endpoint.php?all_colors')
        .then(response => response.json())
        .then(data => {
            const coresOptions = data.map(cor => `<option value="${cor.name.toLowerCase()}">${cor.name}</option>`).join('');
            ...
            document.getElementById('form-editar-usuario').addEventListener('submit', function (event) {
                ...
                fetch('endpoint.php', {
                    method: 'PUT',
                    body: JSON.stringify({ edit_user: true, id: id, nome: nome, email: email, cor: cor }),
                    headers: {
